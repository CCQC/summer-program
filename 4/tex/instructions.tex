...
\vspace{-1cm}
\noindent
Let $\{\f_P\}$ be RHF spatial orbitals with orbital energies $\{\ev_P\}$ and basis expansion coefficients $C_{\mu P}$.
\begin{align}
...
=
  \sum_{IJAB}
  \fr{\ip{\f_I\f_J|\f_A\f_B}\pr{2\cdot\ip{\f_I\f_J|\f_A\f_B} - \ip{\f_I\f_J|\f_B\f_A}}}
     {\ev_I+\ev_J-\ev_A-\ev_B}
\end{align}
...
\begin{enumerate}
  \item Run RHF code to obtain MO coefficients $\{C_{\mu P}\}$ and orbital energies $\{\ev_P\}$.
  \item Transform two-electron integrals from the AO basis $\{\x_\mu\}$ to the MO basis $\{\f_P\}$ (equation \ref{tei-transformation})
